...
void RandomMonster::Update(GameTime* gameTime, Input::Keyboard* keyboard)
{
	...
	velocity->X = rand()%3 -1;
	velocity->Y = rand()%3 -1;
	if (position->X + velocity->X < 0 || position->X + velocity->X > Graphics::GameWindow::GetWidth() - texture->GetWidth())
	{
		velocity->X *= -1;
	}
	if (position->Y + velocity->Y < 0 || position->Y + velocity->Y > Graphics::GameWindow::GetHeight() - texture->GetHeight())
	{
		velocity->Y *= -1;
	}
}